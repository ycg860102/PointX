...
if __name__ == "__main__":
    ...
    stockzf = utility.getStockListPriceTSL(list(stocks["stockID"]),int(begd),int(endd),'close') 
    stockzf = stockzf.pct_change().fillna(0)
    #对涨跌情况进行处理，获取股票期间净值情况
    stockProdDatas = (stockzf.ix[1:,:]+1).cumprod()*1000
    
    Nlist = [1,]
    #计算个股点叉图
    PointX.dealAndPlot(stockProdDatas,Path,Nlist)
    #计算个股相对强弱点叉图
    datas = PointX.DataTORS(stockProdDatas) 
    Path+=u'/个股RS'
